...
inline Value LocalGet::impl(LocalGet& local_get, Instance& instance) {
    ...
    TRACE_VERBOSE(
        ...
        instance.getGlobalState().getDebugInfo().getFormattedLocation(local_get.addr_),
        local_get.local_idx_, local.i32, local.i64);
    return local;
}

inline void LocalSet::impl(LocalSet& local_set, Instance& instance, Value local) {
    instance.getActiveContext().setLocal(local_set.local_idx_, local);
    TRACE_VERBOSE(
        "[{:3}] {}: local.set {}: (i32={}, i64={}) -> ()",
        instance.getActiveContext().getEpilogues().size(),
        instance.getGlobalState().getDebugInfo().getFormattedLocation(local_set.addr_),
        local_set.local_idx_, value.i32, value.i64);
};

//inline Value LocalTee::impl(LocalTee& local_tee, Instance& instance, Value value) {
...
inline Value I32Const::impl(I32Const& i32_const, Instance& instance) {
    TRACE_VERBOSE(
        "[{:3}] {}: i32.const {}: () -> ({})",
        instance.getActiveContext().getEpilogues().size(),
        instance.getGlobalState().getDebugInfo().getFormattedLocation(i32_const.addr_),
