...
Errno MemoryManagementUnit::mapPage(uint32_t ptable_idx, uint32_t virt_addr, uint32_t offset, int32_t prot) {
    if (ptable_idx >= tables_.size() || (virt_addr & WPAGE_MASK) || (offset & WPAGE_MASK))
        return Errno::INVALID_ARGUMENT;
...
Errno MemoryManagementUnit::unmapPage(uint32_t ptable_idx, uint32_t virt_addr, uint32_t* offset_out, int32_t* prot_out) {
    if (ptable_idx >= tables_.size())
        return Errno::INVALID_ARGUMENT;
...
Errno MemoryManagementUnit::getPage(uint32_t ptable_idx, uint32_t virt_addr, uint32_t* offset_out, int32_t* prot_out) {
    if (ptable_idx >= tables_.size())
        return Errno::INVALID_ARGUMENT;
...
Errno PageTable::unmapPage(uint32_t virt_addr, uint32_t* offset, int32_t* prot) {
    uint16_t page_num = virt_addr >> 16;
...
bool PageTable::translate(uint32_t virt_addr, uint32_t& offset, AccessType access_type) {
